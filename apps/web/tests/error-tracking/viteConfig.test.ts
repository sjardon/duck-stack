...
// T028 — R003, NF001, EC001

describe('vite.config — sourcemaps and plugin gated on SENTRY_AUTH_TOKEN (R003, NF001, EC001)', () => {
  it('WHEN SENTRY_AUTH_TOKEN is set THEN build.sourcemap is true and a sentry-vite-plugin entry is included with filesToDeleteAfterUpload and an errorHandler that rethrows', async () => {
  ...
  it('WHEN the source-map upload fails THEN the errorHandler rethrows instead of swallowing the error, so vite build fails (EC001)', async () => {
    process.env.SENTRY_AUTH_TOKEN = 'token-123';
    process.env.SENTRY_ORG = 'my-org';
    process.env.SENTRY_PROJECT = 'my-project';
    process.env.VITE_RELEASE = 'v1.0.0';
...
describe('vite.config — build-only secrets are never exposed to the client bundle (EC006)', () => {
  it('WHEN resolving the sentry-vite-plugin options THEN SENTRY_AUTH_TOKEN, SENTRY_ORG, SENTRY_PROJECT and SENTRY_URL are read only from process.env and are not VITE_-prefixed', async () => {
    process.env.SENTRY_AUTH_TOKEN = 'token-123';
    process.env.SENTRY_ORG = 'my-org';
    process.env.SENTRY_PROJECT = 'my-project';
