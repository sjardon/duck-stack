...
| `apps/services` | Fastify + TypeScript | Backend API. Containerised; deployed to DigitalOcean App Platform (INFRA-008). |
...
| Resend | `apps/services` | Transactional email delivery. Accessed exclusively through the `EmailNotifier` port defined in `@repo/types`; the `ResendEmailNotifier` adapter in `apps/services/src/modules/notifications/providers/` is the only concrete implementation, resolved as a singleton by `resolveNotifier()`. Sends are in-process and fire-and-forget — no queue or separate worker (NOTIFICATIONS-002 replaced the previous AWS SES v2 adapter). |
| Better Stack (error tracking) | `apps/services` | Backend exception reporting, distinct from the log/uptime wiring in `duck-spec/docs/INFRASTRUCTURE.md` (INFRA-011). Accessed exclusively through the `IErrorReporter` port in `apps/services/src/shared/providers/errorReporter.ts`; the `@sentry/node`-backed `SentryErrorReporter` adapter is the only concrete implementation, pointed at Better Stack's Sentry-compatible ingest endpoint via `ERROR_TRACKING_DSN`. Opt-in: a `NoopErrorReporter` is used instead when the DSN is unset (SERVICES-011). |

## Inter-service communication

`apps/web` calls `apps/services` over HTTP using the `VITE_API_URL` environment variable as the base URL. All calls are routed through `api/client.ts` (`apiFetch`). In development `VITE_API_URL` defaults to `http://localhost:3000`; in production it points to the backend's DigitalOcean App Platform public URL (INFRA-008; see `duck-spec/docs/INFRASTRUCTURE.md`).
