...
## `apps/web` — Analytics and feature flag conventions

`lib/analytics.ts` exports `readAnalyticsConfig()` and `initAnalytics()`, called synchronously in `main.tsx` before `createRoot(...).render(...)`, mirroring the `lib/errorTracking.ts` init-module shape: config (`VITE_POSTHOG_KEY`, `VITE_POSTHOG_HOST`) is read from `import.meta.env`, and when the key is absent `initAnalytics()` is a no-op — `posthog-js`'s exported functions are documented no-ops when never initialized, so the rest of the app requires no conditional wiring. `posthog.init()` is wrapped in `try/catch` so an initialization failure never blocks rendering. `capture_pageview: "history_change"` gives automatic screen views; `autocapture: false` and `capture_heatmaps: false` disable automatic and high-frequency capture, leaving `captureEvent(name, properties)` (also exported from `lib/analytics.ts`) as the single call site for recording a named product action. `session_recording.maskAllInputs: true` plus a `maskTextSelector`/`maskTextFn` pair mask all replay text by default; an element opts out of masking explicitly via `data-ph-allow="true"`. `PostHogProvider` (from `@posthog/react`) wraps `<App/>` in `main.tsx` unconditionally with the singleton `posthog` client, regardless of whether `posthog.init()` ran, so flag-resolution hooks can always be called without a missing-context failure.

Flag resolution is exposed through `hooks/useFeatureFlag.ts` (`useFeatureFlag(key): { enabled, isResolved }`), which wraps `@posthog/react`'s `useFeatureFlagEnabled` and collapses its three states — not-configured, unresolved, resolved — so callers get a deterministic safe default (`enabled: false`) whenever the flag cannot be resolved, rather than a blocking or flickering render. `components/flags/FeatureFlagGate.tsx` is the declarative gating primitive (`flag`, `children`, `fallback`, `loading` props): it renders `loading ?? fallback` while `isResolved` is `false` and never renders the enabled branch before removing it. This is the accepted pattern for release-time feature enablement; it is a separate concern from `useEntitlement`/`<EntitlementGate>`, `useQuota`/`<QuotaGate>`, and `useTrialStatus`, which gate on what the customer has paid for.

User attribution is synced from `hooks/use-sync-analytics-user.ts` (`useSyncAnalyticsUser()`), invoked once from the root `App.tsx` component above the router — the same documented cross-cutting exception as `useSyncErrorTrackingUser`. It calls `posthog.identify(user.id)` (identifier only) when a Clerk session is present and `posthog.reset()` on sign-out.
