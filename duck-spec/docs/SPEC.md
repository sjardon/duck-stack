...
**Status:** Plans catalog implemented (SUBS-001). Subscribe/cancel flow implemented (SUBS-002). Lifecycle webhooks implemented (SUBS-003). Pricing page and billing settings UI implemented (SUBS-004). Entitlements and feature gates implemented (SUBS-005). Plan usage quotas implemented (SUBS-006). Quota UI gates implemented (SUBS-007). Free trial mode (backend) implemented (SUBS-008).
...
The module defines a code-level `plan.code → EntitlementName[]` mapping in `entitlements.ts`. `GET /billing/entitlements/me` (authenticated) returns the entitlements resolved from the scope's active subscription, falling back to the `free` plan when no subscription exists. The `requireEntitlement(name)` preHandler factory gates backend routes by entitlement name and caches the resolved entitlement array on `request.entitlements` for the lifetime of the request to avoid redundant database queries. A `canceled` subscription continues to grant its plan's entitlements until `current_period_end` elapses; `past_due` subscriptions retain entitlements unless `STRICT_ENTITLEMENTS_ON_PAST_DUE=true`. On the frontend, `useEntitlement(name)` and `<EntitlementGate name="...">` consume `GET /billing/entitlements/me` via a shared React Query cache entry (5-minute staleTime) and render children or an upgrade CTA fallback respectively.

The module tracks numeric consumption limits via a `plan.code → Record<QuotaName, { soft_limit, hard_limit }>` mapping in `entitlements.ts`, persisted per scope and billing period in the `usage_counters` Supabase table (`user_id`, `org_id`, `quota_name`, `period_start`, `count`; unique constraint on the four-column tuple; scope check constraint). `GET /billing/quotas/me` (authenticated) returns the current count, thresholds, period bounds, and a derived `state` (`normal` | `soft_exceeded` | `hard_exceeded`) for every quota defined on the scope's active plan. The `requireQuota(name)` preHandler factory atomically increments the counter via `INSERT … ON CONFLICT DO UPDATE … RETURNING count` and throws `QuotaExceededError` (HTTP 429, code `QUOTA_EXCEEDED`) when the returned count exceeds the plan's `hard_limit`; if the plan does not define the named quota the route proceeds without inserting a row (unlimited). When no active subscription exists and `SIGNUP_MODE=freemium` (the default), `ensureActiveSubscription` lazily creates a synthetic `free` subscription; this lazy creation is skipped in `free_trial` mode, where `ensureActiveSubscription` returns `null` instead and quota/entitlement consumers treat the scope as plan-less. On the frontend, `useQuota(name)` and `<QuotaGate name="...">` consume `GET /billing/quotas/me` via a shared React Query cache entry (60-second staleTime, refetch on window focus) and render children, a warning overlay, or a blocked fallback based on quota state.

The module supports two signup models via the `SIGNUP_MODE` environment variable (`freemium` | `free_trial`, defaulting to `freemium`). In `free_trial` mode, the Clerk `user.created` webhook creates a `trialing` subscription on the highest-priced active plan for every new user, with a configurable duration (`FREE_TRIAL_DAYS`, defaulting to `14`). The `requireActiveSubscription` preHandler is registered as a global `onRequest` hook after `clerkAuthPlugin`; in `free_trial` mode it blocks expired-trial scopes with HTTP 403 (`TRIAL_EXPIRED`, body `{ trialEndedAt }`). Routes under `/billing/*`, `/webhooks/*`, and `/health` are excluded so that expired-trial users can reach billing pages to select a plan. The lazy `trialing → expired` status transition is applied atomically before any read that may expose subscription state. `GET /billing/subscriptions/me` includes `trial_ends_at` and `days_remaining` in its response when `status === 'trialing'`.
