...
The module tracks numeric consumption limits via a `plan.code → Record<QuotaName, { soft_limit, hard_limit }>` mapping in `entitlements.ts`, persisted per scope and billing period in the `usage_counters` Supabase table (`user_id`, `org_id`, `quota_name`, `period_start`, `count`; unique constraint on the four-column tuple; scope check constraint). `GET /billing/quotas/me` (authenticated) returns the current count, thresholds, period bounds, and a derived `state` (`normal` | `soft_exceeded` | `hard_exceeded`) for every quota defined on the scope's active plan. The `requireQuota(name)` preHandler factory atomically increments the counter via `INSERT … ON CONFLICT DO UPDATE … RETURNING count` and throws `QuotaExceededError` (HTTP 429, code `QUOTA_EXCEEDED`) when the returned count exceeds the plan's `hard_limit`; if the plan does not define the named quota the route proceeds without inserting a row (unlimited). When no active subscription exists, `ensureActiveSubscription` lazily creates a synthetic `free` subscription with `current_period_start = date_trunc('month', now())` before performing the upsert; concurrent creation races are resolved by catching the unique-constraint violation and re-reading the now-existing row. Period rollover is natural: a new `current_period_start` does not match existing rows, so the upsert inserts a fresh row and prior rows are preserved as historical records. Shared types `QuotaName`, `QuotaThresholds`, `QuotaState`, `QuotaUsage`, and `QuotasResponse` are published from `@repo/types`. On the frontend, `useQuota(name)` and `<QuotaGate name="...">` consume `GET /billing/quotas/me` via a shared React Query cache entry (60-second staleTime, refetch on window focus) and render children, a warning overlay, or a blocked fallback based on quota state. `useInvalidateQuotas()` allows consumers to invalidate the cache after quota-consuming mutations. The upgrade CTA inside `<QuotaGate>` resolves the next more expensive plan from the catalog; when the user is already on the top plan an informational message is shown instead.
