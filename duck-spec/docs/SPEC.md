...
**Status:** Fastify base structure in place; postgres.js direct database client in use; environment configuration centralized under typed config objects; file naming normalized to lowercase camelCase throughout; request-scoped `requestId` propagated to all log lines via AsyncLocalStorage mixin.
