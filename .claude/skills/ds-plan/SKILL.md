...
**Size check**: before proceeding to Phase 2, flag the idea as **too large** if ANY are true: (a) more than ~5 distinct user-visible behaviors, (b) compound objective (serves 2+ independent user goals), (c) requires building a foundation AND something on top of it, (d) spans multiple lifecycle phases at once (create + update + delete + webhooks + ...). If too large → go to Phase 1b instead of Phase 2. If the idea is clearly large from the first message, skip Phase 1 clarifying questions and go straight to 1b.

---

### Phase 1b — Decompose (when the idea is too large)

Propose the decomposition yourself — do not ask the user to enumerate slices. Apply:
- **Vertical slices**: each feature delivers a complete observable behavior end-to-end.
- **Minimum first**: the first feature is the smallest independently useful thing.
- **Linear chain**: each feature depends on the previous; avoid converging dependencies.
- **Bounded size**: ~4-7 functional requirements per feature; split further if needed.

Present the proposal immediately as a table:

```
This idea covers a lot of ground. Here's how I'd break it down:
...
Then: *"Does this breakdown make sense? We can adjust titles, re-slice, or add/remove pieces."*

Iterate until confirmed, then proceed to Phase 2 in **multi-feature mode**.
...
**Multi-feature mode**: assign all features to a module at once. If they span multiple modules, note this in the decomposition table.
...
**Multi-feature mode**: read the file once, then assign sequential IDs to all features in decomposition order, reserving them all now.
...
**Multi-feature mode**: present a summary per feature with the final IDs and dependency table, then ask to write all.
...
**Multi-feature mode**: write entries one at a time in decomposition order. For `Dependencias`, use the format `- <ID> — <brief reason>`. Do not recreate the file between entries; create it with the module header only for the first entry if it doesn't exist.
...
- Each decomposed feature must produce something independently observable or testable — reject horizontal slices unless the module has no frontend surface.
- Never write any FEATURES.md entry in multi-feature mode until the user confirms the full decomposition in Phase 4.
...
Features created:
  <ID-1> — <title>
  <ID-2> — <title> (depends on <ID-1>)
  ...

Implement them in order with ds-orchestrate. Start with <ID-1>.
```
